...
const FiltradorPorEstado = ({ states, setStates, statesOriginal }) => {
    ...
    const handleSort = (option) => {
        ...
        switch (option) {
            case 'Todos':
                return setStates(statesOriginal);
            case 'Pendientes de Edición':
                return setStates(statesOriginal.filter((item) => item.status === "pending"));
            case 'Editados':
                return setStates(statesOriginal.filter((item) => item.status === "approved"));
            case 'Denegados':
                return setStates(statesOriginal.filter((item) => item.status === "denied"));
            default:
                break;
        }
    };
...
export default FiltradorPorEstado;
